...
export class WidgetManager extends jupyterlab.WidgetManager {
		...
		this.rendermime.addFactory(
			{
				...
				createRenderer: (options: IRenderMime.IRendererOptions) =>
					new jupyterlab.WidgetRenderer(options, this),
			},
			0,
		);
	...
	protected async loadClass(
		...
		this.logger(
			`WidgetManager: Loading class ${className}:${moduleName}:${moduleVersion}`,
		);
			...
			.catch(async (originalException) => {
				try {
					const loadModuleFromRequirejs =
						...
						this.scriptLoader.widgetsRegisteredInRequireJs.has(
							moduleName,
						);
					...
					if (!loadModuleFromRequirejs) {
						...
						await this.scriptLoader.loadWidgetScript(
							moduleName,
							moduleVersion,
						);
					}
	...
	private sendSuccess(
		...
		try {
			this.scriptLoader.successHandler(
				className,
				moduleName,
				moduleVersion,
			);
	...
	private sendError(
		...
		try {
			this.scriptLoader.errorHandler(
				className,
				moduleName,
				moduleVersion,
				originalException,
