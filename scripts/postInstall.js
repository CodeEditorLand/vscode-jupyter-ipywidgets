...
function fixJupyterLabRenderers() {
	...
	[
		...
		if (
			fileContents.indexOf(textToReplace) === -1 &&
			fileContents.indexOf(textToReplaceWith) === -1
		) {
...
function ignoreTSErrors(relativePath) {
	...
	if (fileContents && !fileContents.includes("// @ts-nocheck")) {
		fs.writeFileSync(filePath, `// @ts-nocheck${EOL}${fileContents}`, {
			encoding: "utf-8",
		});
	}
}
